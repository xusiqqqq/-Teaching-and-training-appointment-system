...
@SpringBootTest
public class TMemberLogMapperTest {

	@Autowired
	private TMemberLogMapper logMapper;
	
	private TMemberLog log = new TMemberLog();
	...
	//根据条件值删除
		...
		int deleteCount = logMapper.deleteByMap(columnMap);
	...
	//查询多条数据
		...
		public void selectManyById() {
			...
			idList.add(4);
			idList.add(5);
			List<TMemberLog> ids = logMapper.selectBatchIds(idList);
		...
		public void selectManyByCase() {
			...
			List<TMemberLog> list = logMapper.selectByMap(columnMap);
	...
	//=====使用Wrapper
		...
		List<Map<String, Object>> list = logMapper.selectMaps(null);
		toPrint("查询", list.size(), list);
		}
		//分页展示
		@Test
		public void selectAllOnMapPage() {
			...
			IPage<Map<String, Object>> pageList = logMapper.selectMapsPage(page, null);
		...
		//根据id查询出实体数据
		public TMemberLog findById(Integer id) {
			log = logMapper.selectById(id);
			return log;
		}
